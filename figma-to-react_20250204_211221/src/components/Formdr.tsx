...
const FormdrGeneralPhy: React.FC = () => {
    ...
    useEffect(() => {
        // Load doctor types and time slots into dropdowns
        const doctorTypeDropdown = document.getElementById('doctor-type-dropdown') as HTMLSelectElement;
        ...
        const loadDoctorTypes = () => {
            const loadingOption = document.createElement('option');
            loadingOption.textContent = 'Loading...';
            doctorTypeDropdown.appendChild(loadingOption);

            fetch('http://127.0.0.1:8000/doctor-types')
                .then(response => response.json())
                .then(items => {
                    doctorTypeDropdown.innerHTML = '<option value="" disabled selected>Select type of Doctor</option>';
                    items.forEach((item: string) => {
                        const option = document.createElement('option');
                        option.value = item;
                        option.textContent = item;
                        doctorTypeDropdown.appendChild(option);
                    });
                })
                .catch(error => {
                    console.error('Doctor types fetch error:', error);
                    doctorTypeDropdown.innerHTML = '<option value="" disabled selected>Error loading options</option>';
                });
        };

        // Initialize doctor types dropdown
        loadDoctorTypes();

        const handleSlotSelection = (item: string) => {
                        ...
                        .then(items => {
                            if (slotDropdownContainer) {
                                ...
                                items.forEach((item: string) => {
                                    ...
                                    option.textContent = item;
                                    option.onclick = () => handleSlotSelection(item);
            ...
            if (validateForm()) {
                try {
                    const formData = {
                        ...
                        doctor_type: doctorTypeDropdown.value,
    ...
    return (
        <div className={`${styles['class-0']} Formdrgeneralphy`} id="form-general-phy">
            ...
            <form className={styles['class-57']} id="appointment-form">
                ...
                <input className={styles['class-59']} id="dob-input" name="dob" placeholder="Please select your DOB" required type="date"/>
                ...
                <input className={styles['class-62']} id="appointment-date-input" name="appointment-date" placeholder="Please select your appointment date" required type="date"/>
                ...
                <select className={styles['class-71']} id="doctor-type-dropdown" name="doctor-type" required>
                    <option value="" disabled selected>Select type of Doctor</option>
                </select>
